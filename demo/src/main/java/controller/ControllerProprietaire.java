    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!errorFlag) {
            try {
                ...
                String strQuery = "INSERT INTO mission (date_mission,time_mission,instruction,id_proprietaire,statut,id_propriete,proprietaire_start,proprietaire_end) VALUES ('"
                        + dateMission + "'," + duration + ",'" + instruction + "'," + idProprietaire + ",1,"
                        + idProperty + "," + plageStart + "," + plageEnd + ");";
                stMission.executeUpdate(strQuery);

                String strGetNewMission = "SELECT * FROM mission JOIN propriete ON id_propriete=propriete_id JOIN statut_mission ON statut=statut_id WHERE id_proprietaire ="
                        + idProprietaire + " AND date_mission ='" + dateMission + "' AND time_mission=" + duration
                        + " AND instruction='" + instruction + "';";
                ResultSet rsNewMission = stMission.executeQuery(strGetNewMission);
                while (rsNewMission.next()) {
                    String adress = rsNewMission.getString(16) + " " + rsNewMission.getString(19) + " "
                            + rsNewMission.getString(20);
                    Mission m = new Mission(rsNewMission.getString("date_mission"),
                            rsNewMission.getDouble("time_mission"), rsNewMission.getString("instruction"),
                            rsNewMission.getDouble("proprietaire_start"), rsNewMission.getDouble("proprietaire_end"),
                            user, user.getPropertyById(idProperty), rsNewMission.getString("nom"));
