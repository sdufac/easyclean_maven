...
public class ControllerCommentaire extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (session.getAttribute("user") instanceof Proprietaire) {
            ...
            strQuery = "INSERT INTO commentaire (idAuthor,idUser,idMission,commentaire,note) VALUES (" + user.getId()
                    + "," + user2.getId() + ",33,\"" + request.getParameter("commentaire") + "\","
                    + Float.valueOf(request.getParameter("note")) + ")";
            ...
            strQuery = "INSERT INTO commentaire (idAuthor,idUser,commentaire,note) VALUES (" + user.getId() + ","
                    + user2.getId() + ",\"" + request.getParameter("commentaire") + "\","
                    + Float.valueOf(request.getParameter("note")) + ")";
            DAOacces bdd = new DAOacces("easy_clean", "toto", "titi");
            try {
                bdd.getConnection().createStatement().executeUpdate(strQuery);
