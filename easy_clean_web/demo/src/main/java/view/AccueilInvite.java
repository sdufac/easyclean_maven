	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			while (rsMissions.next()) {
				String adress = rsMissions.getString("adress") + " " + rsMissions.getString("ville") + " "
						+ rsMissions.getString("code_postal");
				Mission m = new Mission(rsMissions.getString("date_mission"), rsMissions.getDouble("time_mission"),
						rsMissions.getString("instruction"), rsMissions.getDouble("proprietaire_start"),
						rsMissions.getDouble("proprietaire_end"), adress, rsMissions.getInt("id_proprietaire"), 1,
						rsMissions.getString("nom"));
		...
		response.getWriter().append(
				"<h2>Accueil Invité</h2><button type=\"button\" onclick=\"location.href='connection'\"/>Se connecter</button> <button type=\"button\" onclick=\"location.href='createaccount'\"/>Créer un compte</button><hr>");
